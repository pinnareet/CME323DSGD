...
Logger.getLogger("akka").setLevel(Level.OFF)
    ...
    val ini_divide = scala.math.pow(1.0/Rank,0.5)

    // Initialize W and H as random 2D arrays (matrices)

    var W = sc.parallelize(0 to numTrainRow-1).map(x => (x,Array.fill(numTrainRow) { Random.nextDouble *  ini_divide}))
    ...
    var HT = sc.parallelize(0 to numTrainCol-1).map(x => (x,Array.fill(numTrainCol) { Random.nextDouble *  ini_divide }))
    ...
    time{while (iter < maxIter) { //Or until convergence
        ...
        WH = VWHP.mapPartitions(iter => iter.map(b => SGD(b, stepSize, mse, nUpdates))) //Each partition has 1 element RDD
        //WH = VHWP.map(b => SGD(b, stepSize))
        
        println(mse.value / nUpdates.value)
