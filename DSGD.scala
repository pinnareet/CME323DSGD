...
// load in V text file
val trainDat = sc.textFile("/FileStore/tables/9lkfxx6x1464574874711/ratings_tiny-993e4.txt")	//val trainDat = sc.textFile("cme323_final_project/test.txt")
...
def SGD (vwh : (Int, (Iterable[MatrixEntry], Iterable[(Int, Array[Double])], Iterable[(Int, Array[Double])])), stepSize : Double, mse : org.apache.spark.Accumulator[Double], nUpdates : org.apache.spark.Accumulator[Double]) = {
    ...
    for (entry <- VIter) {
        ...
        val WRownew = WIter.filter(row => row._1 == rowID).toList(0)._2
        val HColnew = HIter.filter(col => col._1 == colID).toList(0)._2
        val VMinusWHnew = 2*(entry.value - (WRownew,HColnew).zipped.map(_*_).sum)
        mse += scala.math.pow(VMinusWHnew, 2)
        nUpdates += 1
...
time{while (iter < maxIter) { //Or until convergence
    val stepSize = scala.math.pow((tau + iter),beta)
    val colPerms = strata(iter % numStrata) 
    val mse = sc.accumulator(0.0)
    val nUpdates = sc.accumulator(0.0)
    ...
    WH = VWHP.mapPartitions(iterab => iterab.map(b => SGD(b, stepSize, mse, nUpdates))) //Each partition has 1 element RDD
    ...
    val WHout = WH.collect()
    println(nUpdates.value)
    iter = iter + 1
}}
